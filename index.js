...
function initTheme() {
    ...
    if (toggle) {
        ...
        toggle.addEventListener('click', () => {
            ...
            try { managerInstance.redrawGraphics(); } catch (e) {}
        });
    }
    if (isLight) { try { managerInstance.redrawGraphics(); } catch (e) {} }
}
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    // Ensure footer stays in <body>
    (function ensureFooter() {
        const footer = document.querySelector('.site-footer');
        if (!footer) return;
        if (footer.parentElement !== document.body) {
            try { document.body.appendChild(footer); } catch (e) {}
        }
    ...
    // Initialise rope with sample text
    ...
    if (langSelect) {
        ...
        langSelect.addEventListener('change', async (e) => {
            ...
            try { await window.i18next.changeLanguage(newLang); } catch (err) {}
...
function greetings() {
    try {
        ...
        prompt.print('Initialize the rope, then use Insert / Delete / Index to explore it.');
        prompt.print("Use 'Next' and 'Back' to step through operations.");
        prompt.print("Uncheck 'Enable step-by-step' or click 'Finish' to fast-forward.");
        prompt.print('Enjoy!');
    }
    prompt.print();
}
