...
const _t = (key, vars = {}) => {
    try { return i18next.t(key, vars); } catch {
...
export default class Manager {
    ...
    async waitForUser() {
        ...
        next.style.cssText += ';background-color:#4aff50;color:black;font-weight:bold';
        prev.style.cssText += ';background-color:#fb4a3d;color:black;font-weight:bold';
    ...
    /** Build a demo rope with a sample sentence for the Text Demo tab. */
    initializeTextDemo() {
    ...
    initListeners() {
        ...
        window.addEventListener('journey-finished', () => {
            ...
            const msg = _t('messages.finishedExecution') ?? 'Finished execution.';
        ...
        this.fastForwardCheckbox.addEventListener('change', () => {
            ...
            const msg = this.fastForward
                ? (_t('messages.fastForwardEnabled')  ?? 'Step-by-step will be fast-forwarded.')
                : (_t('messages.fastForwardDisabled') ?? 'Step-by-step execution is enabled.');
            this.prompt.print(msg);
        });
    }
}
