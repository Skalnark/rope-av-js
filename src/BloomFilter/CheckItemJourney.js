    ...
    return true
            ...
            try {
                await this._print(
                    window.i18next
                        ...
                        : `Checking if '${item}' is in the Bloom Filter...`,
                    1500,
                );
            } catch (e) {
                await this._print(
        ...
        for (let i = 0; i < managerInstance.bf.hashCount; i++) {
            ...
            hash.action = ((i) => async (context) => {
                try {
                    await this._print(
                        window.i18next
                            ? window.i18next.t('messages.positionsVerified', {
                                  count: i,
                                  total: managerInstance.bf.hashCount,
                              })
                            : `${i}/${managerInstance.bf.hashCount} positions verified.`,
                        1000,
                    );
                } catch (e) {
                    await this._print(
            ...
            checkBit.action = ((i) => async (context) => {
                ...
                try {
                    await this._print(
                        window.i18next
                            ? window.i18next.t('messages.checkBit', {
                                  position: context[hashName],
                              })
                            : `Check the bit at position ${context[hashName]} in the bit array`,
                    );
                } catch (e) {
                    await this._print(
                        `Check the bit at position ${context[hashName]} in the bit array`,
                ...
                if (bit) {
                    try {
                        await this._print(
                            window.i18next
                                ? window.i18next.t('messages.bitIsOne', {
                                      position: context[hashName],
                                  })
                                : `The bit at position ${context[hashName]} is 1`,
                        );
                        await this._print(
                            window.i18next
                                ? window.i18next.t('messages.itemMightBe')
                                : 'This means the item might be in the Bloom Filter...',
                            2000,
                        );
                    } catch (e) {
                        await this._print(
                    ...
                    try {
                        await this._print(
                            window.i18next
                                ? window.i18next.t('messages.bitIsZero', {
                                      position: context[hashName],
                                  })
                                : `The bit at position ${context[hashName]} is 0`,
                            1000,
                        );
                        await this._print(
                            window.i18next
                                ? window.i18next.t(
                                      'messages.itemDefinitelyNot',
                                      { item },
                                  )
                                : `This means that '${item}' is definitely NOT in the Bloom Filter.`,
                            1000,
                        );
                    } catch (e) {
                        await this._print(
        ...
        finalStep.action = async (context) => {
            try {
                await this._print(
                    window.i18next
                        ...
                        : `${managerInstance.bf.hashCount}/${managerInstance.bf.hashCount} positions verified.`,
                    2500,
                );
            } catch (e) {
                await this._print(
            ...
            try {
                await this._print(
                    window.i18next
                        ? window.i18next.t('messages.allBitsAreOne')
                        : `All the bits at the calculated positions are 1.`,
                );
            } catch (e) {
                await this._print(
                    `All the bits at the calculated positions are 1.`,
            ...
            try {
                await this._print(
                    window.i18next
                        ? window.i18next.t('messages.itemMightBe')
                        : `This means that '${item}' is possibly in the Bloom Filter.`,
                );
            } catch (e) {
                await this._print(
                    `This means that '${item}' is possibly in the Bloom Filter.`,
            ...
            if (managerInstance.bf.elements.includes(item)) {
                try {
                    await this._print(
                        window.i18next
                            ? window.i18next.t('messages.wasAddedEarlier')
                            : 'In fact, it was added earlier!',
                    );
                } catch (e) {
                ...
                try {
                    await this._print(
                        window.i18next
                            ? window.i18next.t('messages.falsePositive')
                            : 'However, it was never added. This is a false positive!',
                    );
                } catch (e) {
                    await this._print(
                        'However, it was never added. This is a false positive!',
