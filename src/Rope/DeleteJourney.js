...
export default class DeleteJourney extends Journey {
    ...
    build(from, to) {
        ...
        const pseudoCode = `procedure delete(from, to):
  ...
  rope = concat(L, R)`;
        ...
        initStep.action = async (ctx) => {
            ...
            const msg =
                t('messages.deleteStart', { from, to: clampedTo }) ??
        ...
        split1.action = async (ctx) => {
            ...
            const msg =
                t('messages.splitAt', { pos: clampedFrom }) ??
        ...
        split2.action = async (ctx) => {
            ...
            const msg =
                t('messages.secondSplit', { rangeLen }) ??
        ...
        mergeStep.action = async (ctx) => {
            ...
            await this._print(
                t('messages.discardMiddle') ?? 'Discarding the deleted range…',
            );
            this._deletedRoot = null;
            ...
            await this._print(
                t('messages.concatRight') ??
                    'Concatenating prefix with right remainder…',
            );
            ...
            draw.renderTree(rope.root);
            const finalMsg =
                t('messages.deleteDone', { result: rope.toString() }) ??
